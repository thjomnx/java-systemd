...
public class LoadCredentialTest {

    @Test(description = "Tests parameterized constructor.")
    public void testConstructor() {
        LoadCredential instance = new LoadCredential(new Object[] {
                "foo",
                "bar"
        }
        );
        ...
        Assert.assertEquals(instance.getName(), "foo");
        Assert.assertEquals(instance.getSource(), "bar");
    }
    ...
    public void testConstructorFailures() {
        new LoadCredential(new Object[0]);
    }
    ...
    public void testBulkProcessing() {
        ...
        List<LoadCredential> list = LoadCredential.list(source);
        ...
        list = LoadCredential.list(source);

        Assert.assertNotNull(list);
        Assert.assertEquals(list.size(), 2);

        LoadCredential item = list.get(0);

        Assert.assertNotNull(item);
        Assert.assertEquals(item.getName(), "foo");
        Assert.assertEquals(item.getSource(), "bar");
